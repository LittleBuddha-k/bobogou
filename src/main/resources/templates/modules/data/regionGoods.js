layui.use(['form', 'table'], function () {
    ...
    table.on('toolbar(regionGoodsTableFilter)', function (obj) {
        if (obj.event === 'add') {  // 监听添加操作
            var index = rc.openSaveDialog("/bobogou/data/regionGoods/form/add", "新建地域商品信息")
            ...
            if (idArr[1]) {
                ...
                ids = idArr[0];
                rc.openSaveDialog('/bobogou/data/regionGoods/form/edit?id=' + ids, "编辑地域商品信息");
            }
            $(window).on("resize", function () {
                layer.full(index);
            });
            ...
            if (idArr[1]) {
                ...
                ids = idArr[0];
                rc.openViewDialog('/bobogou/data/regionGoods/form/view?id=' + ids, "查看地域商品信息");
            }
            $(window).on("resize", function () {
                layer.full(index);
            });
            ...
            if (ids == null || ids == '') {
                ...
                rc.post("/bobogou/data/regionGoods/deleteByPhysics?ids=" + ids, '',function (data) {
                    if(data.code == 200){
                        //执行搜索重载
                        refresh();
                        rc.alert(data.msg);
