layui.use(['form', 'table'], function () {
    ...
    table.on('toolbar(agreementTableFilter)', function (obj) {
        if (obj.event === 'add') {  // 监听添加操作
            var index = rc.openSaveDialog("/bobogou/other/agreement/form/add", "新建协议信息",'55%','50%')
        }
    });
    ...
    table.on('tool(agreementTableFilter)', function (obj) {
        ...
        if (obj.event === 'edit') {  // 监听添加操作
            var index = rc.openSaveDialog("/bobogou/other/agreement/form/edit?id="+ids, "编辑协议信息",'55%','50%')
